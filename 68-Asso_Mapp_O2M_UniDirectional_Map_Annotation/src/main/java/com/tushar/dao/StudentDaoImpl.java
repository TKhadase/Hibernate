...
public class StudentDaoImpl implements IStudentDao {
	...
	public void saveStudentInfo() {
		...
		try (Session ses = HBConnection_improved.getSession();) {
			...
			StudentInfo s1 = new StudentInfo("Tushar", "Khadse", "YTL", Map.of("acc1", p1,"acc2",p2));
	...
	public void getStudentInfo() {
		try (Session ses = HBConnection_improved.getSession();) {
			...
			student.forEach(s -> {
				...
				Map<String, Phones> phone = s.getPhones();
				System.out.println(phone.get("acc1")+"---"+phone.get("acc2"));
			});
		} // try
		catch (HibernateException e) {
			System.out.println("Error while performing operations");
