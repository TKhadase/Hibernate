...
public class StudentDaoImpl implements IStudentDao {
	...
	public void updateStudentInfo() {
		Transaction tx =null;
		System.out.println("StudentDaoImpl.updateStudentInfo()");
		try(Session ses = HBConnection_improved.getSession()) {
			StudentInfo s1 = ses.get(StudentInfo.class, 208, LockMode.UPGRADE_NOWAIT);
			s1.setAddrs("OLD DELHI");
			tx =ses.beginTransaction();
			ses.save(s1);
			...
			System.out.println("updateStudentInfo -HibernateException :: Problem occured while saving");
			he.printStackTrace();
			if(tx!=null || tx.getRollbackOnly()  || tx.getStatus()!=null) {
				tx.rollback();
			}
	...
	public void updateStudentInfo1() {
		Transaction tx =null;
		System.out.println("StudentDaoImpl.updateStudentInfo1()");
		try(Session ses = HBConnection_improved.getSession()) {
			StudentInfo s1 = ses.get(StudentInfo.class, 208, LockMode.UPGRADE_NOWAIT);
			s1.setAddrs("New DELHI");
			tx =ses.beginTransaction();
			ses.save(s1);
			tx.commit();
			System.out.println("record updated successfully");
		}catch(HibernateException he) {
			System.out.println("updateStudentInfo1 -HibernateException :: Problem occured while saving");
			he.printStackTrace();
			if(tx!=null || tx.getRollbackOnly()  || tx.getStatus()!=null) {
				tx.rollback();
			}
